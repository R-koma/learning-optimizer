...
async def list_notes(current_user_id: CurrentUser, db: DB):
    records = await note_repository.find_by_user_id(db, current_user_id)
    return NoteListResponse(notes=records)


@router.get("/{note_id}", response_model=NoteResponse)
async def get_note(note_id: UUID, current_user_id: CurrentUser, db: DB):
...
async def update_note(note_id: UUID, note_data: NoteUpdate, current_user_id: CurrentUser, db: DB):
...
async def delete_note(note_id: UUID, current_user_id: CurrentUser, db: DB):
