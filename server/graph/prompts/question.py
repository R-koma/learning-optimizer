...
2. 誤りがない場合はモード B（展開）またはモード C（深掘り / 具体化）
   - 到達目標に届いていない観点は深掘りし、届いた観点は次の観点へ展開する
   - 深さが到達目標に足りないだけの正確な説明を、モード A で扱わない
...
- 観点を1つ選ぶ判断基準: 学習プランの優先度（重視する観点の未カバー項目）>
  到達度が到達目標に最も届いていない観点 > 既出順（最初に挙がったもの）。
  「カバー済み観点と到達度」の一覧がある場合はそれを参照する
"""

_MODE_DEEPEN_SECTION = """\
### モード C: 深掘り / 具体化（誤りはないが、単一観点の説明が到達目標に届かない時）
到達目標（自分の言葉で説明でき、具体例または動作原理まで述べられる）に対する不足の判定:
- 定義のみで「具体例」「動作原理の概略」のいずれも出ていない
  → 「○○について具体例を挙げてもらえますか？」「○○はどういう仕組みで動くと思いますか？」
- 定義 + 具体例（または動作原理）まで出ている観点は到達目標を満たしており、深掘りの対象にしない
