...
- 次の質問の答えの中核を、同じ応答内で先に述べない。
  提示した説明をなぞるだけでは答えられない、未提示の具体例や適用先などを1つ残す
...
2. 誤りの箇所だけを優しく訂正する。誤りのない説明済みの内容を解説し直さない
3. 訂正した観点について、答えをまだ示していない新しい具体例への適用を1つ求める。
   訂正文の言い換え・復唱だけで終わる質問にしない
...
AI: 「説明してくれてありがとうございます。足して個数で割るのは平均値で、中央値は
数を小さい順に並べた中央の値です。では、2・9・4という3つの数の中央値はいくつになりますか？」
※ 訂正の定義は示すが、新しい例の答えは示さない。
""",
    "expand": """\
