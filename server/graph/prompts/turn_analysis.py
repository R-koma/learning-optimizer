...
2. `response_mode`: 次の AI 応答のモード
   - reinforce: ユーザーの説明に明確な誤り・重大な混同がある場合のみ。
     深さが到達目標に足りないだけの正確な説明は誤りではない
   - deepen: 誤りはないが、直近の説明が単一観点で到達目標に届いていない
   - expand: 誤りがなく、直近の説明が十分。複数観点が一度に列挙され各観点に最低限の定義があれば、
     個々の観点が defined 止まりでも expand を優先する（焦点観点への質問の深さはモード C の基準で決める）
   - 到達目標は exemplified（具体例または動作原理まで述べた）。単一観点の説明が defined 止まりなら
     deepen、exemplified 以上に達していて誤りもなければ expand
3. `selected_aspect`: 次の応答で焦点を当てる観点を 1 つ
   - 選定基準: 重視する観点の未カバー項目 > 到達度が到達目標に最も届いていない既出観点 > 既出順
