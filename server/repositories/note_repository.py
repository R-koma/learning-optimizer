...
async def find_by_user_id(conn: asyncpg.Connection, user_id: str) -> list[asyncpg.Record]:
    query = """--sql
...
async def find_by_id(conn: asyncpg.Connection, note_id: UUID, user_id: str) -> asyncpg.Record | None:
    query = """--sql
...
async def update(
  ...
  """
    ...
    return await conn.fetchrow(query, note_id, user_id, topic, content, summary, status)
